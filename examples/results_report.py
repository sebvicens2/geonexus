...
def render_console(data: dict[str, object], paths: dict[str, list[list[str]]]) -> None:
    ...
    head("3. TOP 10 RISK HOTSPOTS")
    print(f"  {'#':>2}  {'node':<24} {'type':<9} {'risk':>6} {'cen':>5} {'inf':>5} {'den':>5}")
    ...
    head("5. CAUSAL PATHS TO ASSETS  (heuristic asset overlay)")
    print("  Chains traverse real actors → real events → asset (mapped heuristically).")
    ...
    print(f"\n{'=' * 70}")
    print(
        f"EventGraph detected {n_clusters} major geopolitical clusters "
        f"(of {total} communities) and {n_hot} risk hotspots "
        f"from {n_events} real World Observer events."
    )
    print("=" * 70)
...
def render_markdown(data: dict[str, object], paths: dict[str, list[list[str]]]) -> str:
    ...
    lines.append("## 3. Top 10 risk hotspots\n")
    lines.append("| # | Node | Type | Risk | Centrality | Influence | Density |")
    lines.append("| --: | --- | --- | --: | --: | --: | --: |")
    for i, (_, label, typ, s) in enumerate(data["hotspots"], 1):
        lines.append(
    ...
    lines.append(
        "_Assets are attached via a transparent heuristic map "
        "(theatre/category → asset); the chains traverse real actors and events._\n"
    )
    for ticker, (name, _) in ASSETS.items():
