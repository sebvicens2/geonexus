"""Concrete GeoNexus results from the real World Observer sample.
...
No LLM. Every number below is computed by GeoNexus from real events.
...
from geonexus import Asset, AssetType, GeoNexus, Relation, RelationType
...
def add_asset_overlay(g: GeoNexus) -> list[str]:
...
# data collection
# ----------------------------------------------------------------------------- #
def node_type(g: GeoNexus, node_id: str) -> str:
    return wo_kind(g, node_id)


def collect(g: GeoNexus) -> dict[str, object]:
...
def _interpret(g: GeoNexus, entities: list[str], events: list[str], regions: list[str]) -> str:
...
def causal_paths(g: GeoNexus) -> dict[str, list[list[str]]]:
...
def render_console(data: dict[str, object], paths: dict[str, list[list[str]]]) -> None:
    ...
    print(f"\n{'=' * 70}")
    print(
        f"GeoNexus organised {n_events} real World Observer events ({n_days} days) "
...
def render_markdown(data: dict[str, object], paths: dict[str, list[list[str]]]) -> str:
    ...
    lines: list[str] = []
    window, n_days = data["window"]  # type: ignore[misc]
    lines.append("# GeoNexus — World Observer results\n")
    lines.append(
        "_Generated by `examples/results_report.py` from real World Observer "
        "events. No LLM; every figure is computed by GeoNexus._\n"
    )
    lines.append(
